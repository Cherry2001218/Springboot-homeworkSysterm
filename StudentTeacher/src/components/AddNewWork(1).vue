...
<div class="list">
        ...
        <div class="card" v-for="item in list" :key="item.sno"  @click="getInsert(item.cid)" >
             <van-swipe-cell>
            <div class="card-n" >
              <div class="title">
                {{item.wname}}
            ...
            <template #left> 
                <van-button square text="删除" type="danger" color="skyblue" class="delete-button" @click="removeComment(index,item.cid)"></van-button>
             </template>
        </van-swipe-cell>
          </div>
...
<div class="reg" style="margin:16px" >
             <van-button round block type="info" native-type="submit" color="skyblue" @click="goAddWork(top)">增加作业项目</van-button>
          </div>
...
//查询这个课程的作业相关信息
  ...
  methods:  {
removeComment(index,cid)
      {
          this.axios.post("content/remove",
          {
              cid:cid,
          }).then((response)=>{
              this.list.splice(index,1);
              Toast.success("删除这项任务！");
          })
      },
      goAddWork(top)
      {
          this.$router.push('/AddWork/'+top.sno)
      },
      getInsert(cid){
        this.$router.push('/Insert/'+cid);
      },
